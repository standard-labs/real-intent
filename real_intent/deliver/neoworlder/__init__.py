...
class NeoworlderDeliverer(BaseOutputDeliverer):
    ...
    def register_client(
        ...
        """
        Register or update a Real Intent client on NeoWorlder.

        This is a one-time setup step, typically called during customer onboarding.
        The endpoint is idempotent - calling it multiple times is safe.

        Args:
            api_key: NeoWorlder API key (neo-api-access-key).
            base_url: NeoWorlder API base URL.
            real_intent_client_id: Unique identifier for the Real Intent client.
            customer_name: Customer's full name (required).
            customer_email: Customer's email address (required).
            customer_phone: Customer's phone number (optional).
            company_name: Company name (optional).
        ...
        url = f"{base_url}/api/v1/external-access/create-or-update-real-intent-client"

        payload = {
            "real_intent_client_id": real_intent_client_id,
            "customer_information": {
                "name": customer_name,
        ...
        headers = {
            "neo-api-access-key": api_key,
            "Content-Type": "application/json",
        }

        log("debug", f"Registering/updating NeoWorlder client: {real_intent_client_id}")

        response = requests.post(url, json=payload, headers=headers, timeout=TIMEOUT_SECONDS)

        log("trace", f"Raw response: {response.text}, status_code: {response.status_code}")

        cls._handle_response_errors_static(response, "register_client", real_intent_client_id)

        log("info", f"Successfully registered/updated NeoWorlder client: {real_intent_client_id}")
        return cls._parse_response_static(response)
